...
pub trait EntityManager {
    ...
    async fn find(&self, openapi: &OpenAPI, schema_name: &str, key: &Value, order_by: &Vec<String>) -> Result<Vec<Value>, Box<dyn std::error::Error>>;
    async fn find_one(&self, openapi: &OpenAPI, schema_name: &str, key: &Value) -> Result<Option<Box<Value>>, Box<dyn std::error::Error>>;
